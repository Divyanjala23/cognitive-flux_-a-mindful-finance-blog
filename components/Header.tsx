...
const ActiveNavLink: React.FC<{ to: string; children: React.ReactNode; onClick?: () => void }> = ({
  to,
  children,
  onClick,
}) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mt-2 text-[10px] text-[var(--text-color-muted)]">
          Tip: press <kbd className="rounded bg-black/10 px-1">Ctrl</kbd>/<kbd className="rounded bg-black/10 px-1">⌘</kbd>+<kbd className="rounded bg-black/10 px-1">K</kbd> to open fast
        </div>
      </div>
    </div>
  );
...
const Header: React.FC<HeaderProps> = ({ currentUser, onLogout, theme, toggleTheme }) => {
  ...
  // Header classes
  const headerBase = "sticky top-0 z-50 transition-all duration-300 will-change-transform";
  const translateClass = dir === "down" && isScrolled ? "-translate-y-full" : "translate-y-0";
  const headerBgClass =
    isScrolled || isMenuOpen
      ? "bg-[var(--header-bg)] backdrop-blur-lg border-b border-[var(--card-border)] shadow-[0_6px_30px_rgba(15,23,42,0.06)]"
      : "bg-transparent";
  const linkColorClass =
    "text-[var(--text-color-muted)] hover:text-[var(--heading-color)] transition-colors";
  const mobileLinkColorClass = `block ${linkColorClass} py-2 rounded-md px-3`;

  const NavLinks: React.FC<{ isMobile?: boolean }> = ({ isMobile }) => (
    <>
      <ActiveNavLink to="/" onClick={() => isMobile && setIsMenuOpen(false)}>
        Home
      </ActiveNavLink>
      <ActiveNavLink to="/about" onClick={() => isMobile && setIsMenuOpen(false)}>
        About
      </ActiveNavLink>
      <ActiveNavLink to="/articles" onClick={() => isMobile && setIsMenuOpen(false)}>
        Articles
      </ActiveNavLink>
      ...
      {currentUser ? (
        <>
            ...
            <div className="relative">
              ...
              {userOpen && (
                <div
                  ...
                  role="menu"
                >
                  <Link to="/profile" className="block px-3 py-2 text-sm text-[var(--text-color)] hover:bg-black/5 dark:hover:bg-white/5" role="menuitem">
                    Profile
                  </Link>
                  <Link to="/settings" className="block px-3 py-2 text-sm text-[var(--text-color)] hover:bg-black/5 dark:hover:bg-white/5" role="menuitem">
                    Settings
                  </Link>
                  <button
                    onClick={onLogout}
                    className="block w-full text-left px-3 py-2 text-sm text-[var(--text-color)] hover:bg-black/5 dark:hover:bg-white/5"
                    role="menuitem"
                  >
                    Logout
                  </button>
          ...
          {isMobile && (
            <button onClick={onLogout} className={`block w-full text-left ${mobileLinkColorClass}`}>
              Logout
            </button>
          )}
        </>
        ...
        <>
          {/* Desktop: separate, attractive buttons */}
          {!isMobile && (
            <div className="ml-2 flex items-center gap-2">
    <Link
      to="/login"
      className="inline-flex h-9 items-center justify-center rounded-xl px-4 text-sm font-semibold
                 border border-[color-mix(in_oklab,var(--c0)_30%,transparent)]
                 text-[var(--c0)] bg-[color-mix(in_oklab,var(--c0)_8%,transparent)]
                 hover:bg-[color-mix(in_oklab,var(--c0)_12%,transparent)] transition-colors"
    >
      ...
      style={{
        background:
          "linear-gradient(135deg, color-mix(in oklab, var(--c0) 80%, transparent), color-mix(in oklab, var(--c2) 80%, transparent))",
        boxShadow: "0 8px 24px color-mix(in oklab, var(--c0) 24%, transparent)",
      }}
    >
      <span className="leading-none">Sign up</span>
          ...
          {/* Mobile fallback: standard links */}
          {isMobile && (
            <>
              <ActiveNavLink to="/login" onClick={() => setIsMenuOpen(false)}>
                Login
              </ActiveNavLink>
              <ActiveNavLink to="/signup" onClick={() => setIsMenuOpen(false)}>
                Signup
              </ActiveNavLink>
            </>
          )}
      ...
      <header className={`${headerBase} ${headerBgClass} ${translateClass}`}>
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <Link
              to="/"
              className="flex items-center space-x-3 text-[var(--heading-color)] hover:text-emerald-400 transition-colors"
            >
